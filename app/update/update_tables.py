...
def process_mitocarta(in_file, out_file):
    ...
    mitocarta = pd.read_excel("app/update/data/raw/{}".format(in_file),
                              sheet_name=1)
    mitocarta = mitocarta[["EnsemblGeneID", "Symbol", "Description",
...
def process_hpo_disgenphen(in_file, out_file, mitocarta_file):
    ...
    hpo = pd.read_csv("app/update/data/raw/{}".format(in_file),
                      sep="\t", skiprows=1,
                      names=["disease_id", "gene_symbol", "entrez_gene_id",
                             "hpo_id", "hpo_term_name"])
...
def process_hpo_genphen(in_file, out_file, mitocarta_file):
    ...
    hpo = pd.read_csv("app/update/data/raw/{}".format(in_file),
                      sep="\t", skiprows=1,
                      names=["entrez_gene_id", "gene_symbol", "hpo_term_name",
                             "hpo_id"])
    # mitocarta = pd.read_csv("app/update/data/tables/{}".format(mitocarta_file))
    # hpo = hpo[hpo.gene_symbol.isin(mitocarta.gene_symbol)]
...
def process_hpo_phengen(in_file, out_file, mitocarta_file):
    ...
    hpo = pd.read_csv("app/update/data/raw/{}".format(in_file),
                      sep="\t", skiprows=1,
                      names=["hpo_id", "hpo_term_name", "entrez_gene_id",
                             "gene_symbol"])
    # mitocarta = pd.read_csv("app/update/data/tables/{}".format(mitocarta_file))
    # hpo = hpo[hpo.gene_symbol.isin(mitocarta.gene_symbol)]
...
def process_omim(in_file, out_file):
    """
    Process the dataset downloaded from Omim to produce the related
    database table.
    ...
    omim = pd.read_csv("app/update/data/raw/{}".format(in_file), sep="\t",
                       skiprows=3, comment="#",
...
def process_orphanet(in_file, out_file):
    """
    Process the dataset downloaded from Orphanet to produce the related
    database table.
...
def process_disgenet_gene(in_file, out_file, mitocarta_file):
    """
    Process the gene-disease association file downloaded from Disgenet to
    produce the related
    database table.
    ...
    disge.rename({"geneId": "entrez_gene_id", "geneSymbol": "gene_symbol",
                  "diseaseId": "umls_disease_id", "diseaseName": "disease_name"},
                 axis=1, inplace=True)
    mitocarta = pd.read_csv("app/update/data/tables/{}".format(mitocarta_file))
...
def process_disgenet_vars(in_file, out_file):
    """
    Process the variant-disease association file downloaded from Disgenet
    to produce the related database table.
...
def process_disgenet_maps(in_file, out_file):
    """
    Process the disease mappings file downloaded from Disgenet to produce
    ...
    disge = pd.read_csv("app/update/data/raw/{}".format(in_file),
                        sep="|", skiprows=1,
                        names=["umls_disease_id", "disease_name", "vocabulary",
                               "disease_id", "alt_disease_name"])
...
def create_diseases(hpo_file, omim_file, orpha_file, out_file):
    """
    Create the Diseases table by grepping diseases from the HpoDisGenePhen
    table.
    ...
    for el in dis_ids:
        ...
        if resource == "OMIM":
            ...
            continue
        diseases = diseases.append(pd.DataFrame({"disease_id": [el],
...
def create_phenotypes(hpo_file, out_file):
    """
    Create the Phenotypes table by grepping phenotypes from the HpoDisGenePhen
    table.
...
def perform_download_data(sources):
    """
    Download the needed data from the web and save them to app/update/data/raw/.
...
def perform_create_tables(sources):
    """
    Process all the needed data after they have been downloaded from the
    web to create the final tables that will be uploaded to the database,
    and save them to app/update/data/tables/.
